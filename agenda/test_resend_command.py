...
from agenda.notifications import ResendDeliveryError
...
class ResendCommandTests(SimpleTestCase):
    @patch('agenda.management.commands.test_resend.send_transactional_email')
    ...
    @patch('agenda.management.commands.test_resend.send_transactional_email', return_value='email_123')
    def test_explicit_recipient_sends(self, send):
        call_command('test_resend', to='owner@example.test', stdout=StringIO())
        self.assertEqual(send.call_args.args[2], 'owner@example.test')
    ...
    @patch('agenda.management.commands.test_resend.send_transactional_email', side_effect=ResendDeliveryError('secret-detail'))
